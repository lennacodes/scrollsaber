...
(function () {
  ...
  function loadAudioBuffer(url, cb) {
    fetch(url)
      ...
      .then(decoded => cb(decoded))
      .catch(() => {});
  }

  function setSoundEnabled(on) {
    soundEnabled = on;
    if (on && !ignitionBuffer) {
      loadAudioBuffer(chrome.runtime.getURL("sounds/eject.ogg"), buf => { ignitionBuffer = buf; });
      loadAudioBuffer(chrome.runtime.getURL("sounds/retract.ogg"), buf => { retractionBuffer = buf; });
    }
  }
  ...
  function adjustFixedElements() {
    // Use TreeWalker to skip subtrees early instead of querySelectorAll("*")
    const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_ELEMENT, {
      acceptNode(node) {
        if (node.id === "scrollsaber-track") return NodeFilter.FILTER_REJECT;
        if (node.dataset.scrollsaberAdjusted) return NodeFilter.FILTER_SKIP;
  ...
  function queueAdjust() {
    ...
    adjustTimer = setTimeout(adjustFixedElements, 500);
  }
