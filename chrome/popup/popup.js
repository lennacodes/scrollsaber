...
function updateDualEnabled(mode) {
  if (mode === "eject") {
    ...
    if (dualToggle.checked) {
      ...
      chrome.storage.local.set({ saberMaul: false });
    }
  }
}

// Load saved state
chrome.storage.local.get(["saberColor", "saberCustomColor", "saberCustomHue", "saberCustomBright", "saberHilt", "saberFlicker", "saberLeftHand", "saberMode", "saberSound", "saberVolume", "saberWidth", "saberMaul"]).then((result) => {
...
modeBtns.forEach((btn) => {
  btn.addEventListener("click", () => {
    chrome.storage.local.set({ saberMode: btn.dataset.mode });
...
colorBtns.forEach((btn) => {
  btn.addEventListener("click", () => {
    chrome.storage.local.set({ saberColor: btn.dataset.color });
...
customToggle.addEventListener("click", () => {
  ...
  if (isOpen) {
    ...
    chrome.storage.local.set({ saberColor: "custom", saberCustomColor: hex });
    colorBtns.forEach((b) => b.classList.remove("active"));
    customWrap.classList.add("active");
  }
});
...
hiltBtns.forEach((btn) => {
  btn.addEventListener("click", () => {
    chrome.storage.local.set({ saberHilt: btn.dataset.hilt });
...
flickerToggle.addEventListener("change", () => {
  chrome.storage.local.set({ saberFlicker: flickerToggle.checked });
});

// Left-hand toggle
lefthandToggle.addEventListener("change", () => {
  chrome.storage.local.set({ saberLeftHand: lefthandToggle.checked });
});

// Sound FX toggle
soundToggle.addEventListener("change", () => {
  chrome.storage.local.set({ saberSound: soundToggle.checked });
});

// Dual mode toggle
dualToggle.addEventListener("change", () => {
  chrome.storage.local.set({ saberMaul: dualToggle.checked });
});
...
volumeSlider.addEventListener("input", () => {
  chrome.storage.local.set({ saberVolume: parseInt(volumeSlider.value) });
});

// Width slider
widthSlider.addEventListener("input", () => {
  chrome.storage.local.set({ saberWidth: parseInt(widthSlider.value) });
});
